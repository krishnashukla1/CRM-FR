...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.role === "admin") {
      try {
        ...
        setServerError("❌ Could not verify admin limit");
        return;
      }
    }
  ...
  useEffect(() => {
    const checkAdminLimit = async () => {
      try {
        ...
        console.log("Admin limit check failed");
      }
    };
    checkAdminLimit();
  }, []);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-tr from-[#667eea] via-[#764ba2] to-[#6b21a8] px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-2xl relative z-10 bg-white/20 backdrop-blur-2xl text-black rounded-3xl shadow-[0_8px_30px_rgba(0,0,0,0.3)] p-10 border border-white/30">

        <div className="mb-6 text-center">
          <img
            src="/d1.png"
            alt="FareBuzzer Logo"
            className="object-contain h-20 mx-auto mb-2"
          />
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex gap-4">
            <div className="relative flex-1">
              <input
                ...
                onChange={handleChange}
                placeholder=" "
                className="w-full px-4 py-3 text-black rounded-lg bg-white/90 peer focus:ring-2 focus:ring-yellow-400"
              />
              <label className="absolute left-4 -top-2.5 text-sm bg-white/90 px-1 rounded">
                First Name *
              </label>
              {errors.firstName && (
                <p className="text-sm text-black">{errors.firstName}</p>
              )}
            </div>
            ...
            <div className="relative flex-1">
              ...
              {errors.lastName && (
                <p className="text-sm text-black">{errors.lastName}</p>
              )}
            </div>
          </div>
          ...
          <div className="relative">
            <input
              ...
              onChange={handleChange}
              placeholder=" "
              className="w-full px-4 py-3 text-black rounded-lg bg-white/80 peer focus:ring-2 focus:ring-yellow-400"
            />

            <label className="absolute left-4 -top-2.5 text-sm bg-white/80 px-1 rounded">
              Password *
            </label>
          ...
          <select
            ...
            value={form.role}
            onChange={handleChange}
            className="w-full px-4 py-3 text-black rounded-lg cursor-pointer bg-white/90 focus:ring-2 focus:ring-yellow-400"
          >
            <option value="user">User</option>
          ...
          {/* Terms & checkbox */}
          <label className="flex items-center gap-2 mt-5 text-sm text-white cursor-pointer">
            <input
              type="checkbox"
              name="agreed"
              checked={form.agreed}
          ...
          {errors.agreed && (
            <p className="text-sm text-black">{errors.agreed}</p>
          )}

          {/* Submit button */}
          <button
            type="submit"
            className="w-full py-3 font-semibold text-black transition bg-yellow-400 rounded-lg cursor-pointer hover:bg-yellow-500"
          >
            Sign Up
          </button>
        </form>
