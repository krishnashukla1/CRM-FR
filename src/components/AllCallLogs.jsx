...
const AllCallLogs = () => {
  const [callLogs, setCallLogs] = useState([]);
  const [allCallLogs, setAllCallLogs] = useState([]); // NEW: Store all data for filtering
  const [filteredLogs, setFilteredLogs] = useState([]);
  const [profitSummary, setProfitSummary] = useState({
    byEmployee: {},
    byMonth: {},
  ...
  // NEW: Function to fetch all data for filtering
  const fetchAllDataForFiltering = async () => {
    try {
      ...
      console.error('Error fetching all call logs for filtering:', err);
    }
  };

  // UPDATED: Fetch call logs with all data fetching
  const fetchCallLogs = useCallback(async (page = 1) => {
    try {
      setLoadingPage(true);
      const logsRes = await API.get(`/call-logs?page=${page}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      ...
      if (logsRes.data.success && Array.isArray(logsRes.data.data)) {
        ...
        // NEW: Fetch all data for filtering on first page load
        if (page === 1) {
        ...
        setCallLogs([]);
        setFilteredLogs([]);
        setAllCallLogs([]); // Reset all data too
        setProfitSummary({ byEmployee: {}, byMonth: {}, total: 0 });
        setDetailedStats({
          callDirections: {},
          callCategories: {},
  ...
  // UPDATED: Apply filters function with improved searching
  const applyFilters = useCallback(() => {
    let results = allCallLogs.length > 0 ? allCallLogs : callLogs;

    // If no filters are active and no search term, show current page data
    const isFilteringActive = searchTerm || filters.callCategory || filters.wasSaleConverted || 
                             filters.employeeId || filters.startDate || filters.endDate || 
                             (selectedMonth && selectedYear);

    if (!isFilteringActive) {
      // No filtering, just show current page data with sorting
      results = [...callLogs];
    } else {
    ...
    calculateProfitSummary(results, targets);
    calculateDetailedStats(results);
  }, [allCallLogs, callLogs, searchTerm, filters, selectedMonth, selectedYear, sortConfig, targets]);
  ...
  return (
    <div className="h-screen min-h-screen p-4 overflow-y-auto md:p-8 bg-gray-50">
      <div className="mx-auto max-w-8xl">
        {/* Header */}
        <div className="flex items-center justify-center max-w-8xl max-h-20 mx-auto p-6 
    bg-gradient-to-r from-blue-600 to-indigo-700 
    rounded-xl shadow-lg hover:scale-[1.02] transition-transform duration-300 mb-8">
          <FiPhone className="mr-4 text-4xl text-white" />
          ...
          <div className="p-4 bg-white border border-gray-200 rounded-lg shadow-md">
            ...
            <div className="flex justify-end mt-4 space-x-3">
              <button
                onClick={downloadDetailedReport}
                disabled={filteredLogs.length === 0}
                className={`flex cursor-pointer items-center px-4 py-2 bg-indigo-600 !text-white rounded-md hover:bg-indigo-700 ${
                  filteredLogs.length === 0 ? 'opacity-50 cursor-not-allowed' : ''
                }`}
              >
                <FiDownload className="mr-2" />
        ...
        {/* Summary Cards, Detailed Statistics, Employee Performance, Main Table, and Pagination */}
        {/* ... (keep all the existing JSX for these sections) ... */}
         ...
         {/* Summary Cards */}
         <div className="grid grid-cols-1 gap-6 mb-8 lg:grid-cols-4 ">
           <div className="p-6 bg-white border border-gray-200 rounded-lg shadow-md max-h-30">
             <div className="flex items-center justify-between">
               <div>
                 <p className="text-sm font-medium text-gray-500">Total Calls</p>
                 <p className="text-3xl font-bold text-gray-800">{filteredLogs.length}</p>
...
<div className="mb-10 overflow-hidden bg-white border border-gray-100 shadow-xl rounded-2xl">
  {/* Header */}
  <div className="p-3 border-b border-gray-200 max-h-15 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500">
    <h3 className="text-xl font-bold tracking-wide text-white">📊 Detailed Statistics</h3>
  </div>

  {/* Grid Content */}
  <div className="grid grid-cols-1 gap-6 p-6 md:grid-cols-2 lg:grid-cols-5">
    {/* Card Item */}
    <div className="p-4 transition-all shadow-sm bg-gray-50 rounded-xl hover:shadow-md">
      <h4 className="pb-1 mb-3 font-semibold text-gray-800 border-b">Call Directions</h4>
      <ul className="space-y-2 text-sm">
